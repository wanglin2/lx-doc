...
var ExportDialog = function(editorUi)
{
	...
	transparentCheckbox.setAttribute("readOnly", true);
...
ExportDialog.exportFile = function(editorUi, name, format, bg, s, b, dpi)
{
	...
	{
		var blob = new Blob([mxUtils.getXml(graph.getSvg(bg, s, b))], {
            type: 'image/svg+xml'
        })
		var obUrl = URL.createObjectURL(blob)
		downloadFile(obUrl, name)
		// ExportDialog.saveLocalFile(editorUi, mxUtils.getXml(graph.getSvg(bg, s, b)), name, format);
	}
	else if (format == 'png')
	{
		exportSvgToPng(mxUtils.getXml(graph.getSvg(bg, s, b)), name)
		// ExportDialog.saveLocalFile(editorUi, mxUtils.getXml(graph.getSvg(bg, s, b)), name, format);
	}
    else
    {
