...
EditorUi = function (editor, container, lightbox) {
  ...
  graph
    .getSelectionModel()
    .addListener(mxEvent.CHANGE, this.selectionStateListener)
  ...
  if (this.editor.chromeless && !this.editor.editable) {
    ...
    graph.panningHandler.isForcePanningEvent = function (me) {
      ...
      while (source != null && source != graph.container) {
        if (source.nodeName === 'foreignObject' || source.nodeName === 'text') {
          return false
        }
  ...
  if (!graph.standalone) {
      ...
      if (mxClient.IS_IOS) {
        ...
        if (ver != null && ver[0] >= 16) {
          mxUtils.setPrefixedStyle(
            this.menubarContainer.style,
            'userSelect',
            'none'
          )
          mxUtils.setPrefixedStyle(
            this.diagramContainer.style,
            'userSelect',
            'none'
          )
          mxUtils.setPrefixedStyle(
          ...
          mxUtils.setPrefixedStyle(
            this.formatContainer.style,
            'userSelect',
            'none'
          )
          mxUtils.setPrefixedStyle(
            this.footerContainer.style,
            'userSelect',
            'none'
          )
      ...
      mxUtils.bind(this, function (sender, evt) {
        ...
        if (this.updateDefaultStyle || force) {
          graph.copyCellStyles(
            ...
            evt.getProperty('keys'),
            evt.getProperty('values'),
            graph.currentVertexStyle,
            graph.currentEdgeStyle,
            vertexStyleIgnored,
            edgeStyleIgnored
    ...
    this.resizeHandler = mxUtils.bind(this, function () {
      ...
      resizeThread = window.setTimeout(
...
EditorUi.parsePng = function (f, fn, error) {
  ...
  if (
    fread(f, 8) !=
    String.fromCharCode(137) + 'PNG' + String.fromCharCode(13, 10, 26, 10)
  ) {
    if (error != null) {
      error()
    }
...
EditorUi.prototype.hsplitPosition =
  screen.width <= Editor.smallScreenWidth
    ? 0
  ...
  if (result.cells.length > 1 && result.cell) {
    ...
    if (parent != null && table != null) {
      ...
      while (index < cells.length - 1) {
        ...
        if (
          next == model.getChildAt(parent, col + colspan) &&
          state != null &&
 ...
 * Returns information about the current selection.
 */
EditorUi.prototype.updateSelectionStateForCell = function (
  result,
  cell,
  cells,
  initial
        ...
        thread = window.setTimeout(
          mxUtils.bind(this, function () {
            ...
            var temp = graph.getCompositeParent(
              this.hoverIcons.currentState != null
                ? this.hoverIcons.currentState.cell
                : null
            )
  ...
  getInsertLocationFn =
    getInsertLocationFn != null
      ...
      : function (cells) {
          ...
          return new mxPoint(
            graph.snap(
              Math.round(x / graph.view.scale) - graph.view.translate.x - w
            ),
            graph.snap(
              Math.round(y / graph.view.scale) - graph.view.translate.y - h
            )
          )
        }
  ...
  if (cells != null && cells.length > 0) {
    ...
    var addCell = mxUtils.bind(this, function (cell) {
      ...
      if (style != null && urlParams['sketch'] != '1') {
        ...
        this.sidebar.graph.pasteCellStyles(
          [cell],
          graph.currentVertexStyle,
          graph.currentEdgeStyle
        )
        ...
        node.appendChild(
          this.sidebar.createVertexTemplateFromCells(
            ...
            mxUtils.bind(this, function (evt) {
              if (
                mxEvent.isShiftDown(evt) &&
                (source != null || !graph.isSelectionEmpty())
              ) {
    ...
    for (
      var i = 0;
      i < (hovering ? Math.min(cells.length, 4) : cells.length);
      i++
    ) {
      addCell(cells[i])
    }
 ...
 * Creates a temporary graph instance for rendering off-screen content.
 */
EditorUi.prototype.getCellsForShapePicker = function (
  cell,
  hovering,
  showEdges
) {
  ...
  var createEdge = mxUtils.bind(this, function (style, y, value) {
    var cell = new mxCell(
      value || '',
      new mxGeometry(0, 0, graph.defaultEdgeLength + 20, 0),
      style
    )
    cell.geometry.setTerminalPoint(new mxPoint(0, 0), true)
    cell.geometry.setTerminalPoint(
      new mxPoint(cell.geometry.width, y != null ? y : 0),
      false
    )
    cell.geometry.points =
      y != null ? [new mxPoint(cell.geometry.width / 2, y)] : []
      ...
      if (mxEvent.isAltDown(evt)) {
        ...
        try {
          var nesting =
            ...
            graph.cellEditor.isTextSelected()
          ...
          if (nesting) {
            ...
            document.execCommand(
              mxEvent.isShiftDown(evt) ? 'outdent' : 'indent',
              false,
              null
            )
          }
          // Shift+tab applies value with cursor
            ...
            graph.cellEditor.insertTab(
              !graph.cellEditor.isContentEditing() ? 4 : null
            )
          }
        } catch (e) {
          // ignore
 ...
 */
EditorUi.prototype.updateCssForMarker = function (
  markerDiv,
  prefix,
  shape,
  marker,
...
EditorUi.prototype.updatePasteActionStates = function () {
  ...
  paste.setEnabled(
    this.editor.graph.cellEditor.isContentEditing() ||
      (((!mxClient.IS_FF && navigator.clipboard != null) ||
        !mxClipboard.isEmpty()) &&
        graph.isEnabled() &&
        !graph.isCellLocked(graph.getDefaultParent()))
  )
          ...
          if (pagesMenu != null) {
            mxEvent.addListener(
              ...
              mxUtils.bind(this, function (evt) {
                ...
                menu.popup(
                  offset.x,
                  offset.y + pageInfo.offsetHeight,
                  null,
                  evt
                )
        ...
        if (this.container != null && mxUtils.hasScrollbars(this.container)) {
          ...
          if (!this.autoTranslate) {
            ...
            if (tr.x != tx || tr.y != ty) {
              ...
              if (!skipScroll) {
                this.container.scrollLeft += Math.round(
                  (tr.x - tx) * this.view.scale
                )
                this.container.scrollTop += Math.round(
                  (tr.y - ty) * this.view.scale
                )
              }
    ...
    if (zoomIn) {
      if (this.view.scale * this.cumulativeZoomFactor <= 0.15) {
        ...
        this.cumulativeZoomFactor =
          Math.round(this.view.scale * this.cumulativeZoomFactor * 100) /
          100 /
          this.view.scale
      }
    } else {
          ...
          while (source != null) {
            if (source == graph.container) {
              ...
              // Slower zoom for pinch gesture on touch screens
                ...
                factor =
                  1 + (Math.max(1, Math.abs(evt.movementY)) / 20) * (factor - 1)
                delay = -1
              }
...
EditorUi.prototype.addBeforeUnloadListener = function () {
  ...
  window.onbeforeunload = mxUtils.bind(this, function () {
    console.log('onbeforeunload', this.editor.modified)
    if (!this.editor.isChromelessView()) {
  ...
  if (!this.editor.extendCanvas) {
    ...
    if (mxUtils.hasScrollbars(c)) {
      if (graph.pageVisible) {
        ...
        c.scrollLeft =
          Math.floor(Math.min(pad.x, (c.scrollWidth - c.clientWidth) / 2)) - 1
        ...
        if (bounds.width > 0 && bounds.height > 0) {
          if (bounds.x > c.scrollLeft + c.clientWidth * 0.9) {
            c.scrollLeft = Math.min(
              bounds.x + bounds.width - c.clientWidth,
              bounds.x - 10
            )
          }
          ...
          if (bounds.y > c.scrollTop + c.clientHeight * 0.9) {
            c.scrollTop = Math.min(
              bounds.y + bounds.height - c.clientHeight,
              bounds.y - 10
            )
          }
        }
      ...
      var dy = graph.pageVisible
        ? 0
        : Math.max(0, (c.clientHeight - b.height) / 4)
 ...
 * Loads the stylesheet for this graph.
 */
EditorUi.prototype.installResizeHandler = function (
  dialog,
  resizable,
  destroy
) {
...
EditorUi.prototype.setPageFormat = function (value, ignorePageVisible) {
  ignorePageVisible =
    ignorePageVisible != null ? ignorePageVisible : urlParams['sketch'] == '1'
...
EditorUi.prototype.updateActionStates = function () {
  ...
  this.actions
    .get('swap')
    .setEnabled(ss.cells.length == 2 && ss.vertices.length == 2)
  this.actions
    .get('pasteSize')
    .setEnabled(this.copiedSize != null && ss.vertices.length > 0)
  this.actions
    .get('pasteData')
    .setEnabled(this.copiedValue != null && ss.cells.length > 0)
  this.actions
  ...
  this.actions
    .get('enterGroup')
    .setEnabled(ss.cells.length == 1 && graph.isValidRoot(ss.cells[0]))
  ...
  this.actions
    .get('openLink')
    .setEnabled(
      ss.cells.length == 1 && graph.getLinkForCell(ss.cells[0]) != null
    )
  ...
  var foldable = ss.vertices.length == 1 && graph.isCellFoldable(ss.vertices[0])
  ...
  this.menus
    .get('navigation')
    .setEnabled(ss.cells.length > 0 || graph.view.currentRoot != null)
  this.menus.get('layout').setEnabled(unlocked)
  this.menus.get('insert').setEnabled(unlocked)
  this.menus.get('direction').setEnabled(ss.unlocked && ss.vertices.length == 1)
  this.menus.get('distribute').setEnabled(ss.unlocked && ss.vertices.length > 1)
  this.menus.get('align').setEnabled(ss.unlocked && ss.cells.length > 0)
 ...
 */
EditorUi.prototype.pickColor = function (
  color,
  apply,
  defaultColor,
  defaultColorValue
  ...
  var dlg = new ColorDialog(
    this,
    color != 'default' ? mxUtils.rgba2hex(color) || 'none' : color,
    function (color) {
...
EditorUi.prototype.writeHtmlToClipboard = function (html, error) {
  var cbi = new ClipboardItem({
    'text/plain': new Blob([Editor.convertHtmlToText(html)], {
      type: 'text/plain'
    }),
    'text/html': new Blob([html], { type: 'text/html' })
  })
                  ...
                  .then(
                    mxUtils.bind(this, function (value) {
                      try {
                        ...
                        var asHtml =
                          elt.getAttribute('data-type') != 'text/plain'
                        ...
                        try {
                          ...
                          var tmp =
                            spans != null && spans.length > 0
                              ? mxUtils.trim(
                                  decodeURIComponent(spans[0].textContent)
                                )
...
EditorUi.prototype.parseHtmlData = function (data) {
  ...
  if (data != null && data.length > 0) {
    ...
    elt.innerHTML =
      (hasMeta ? '<meta charset="utf-8">' : '') + Graph.sanitizeHtml(data)
    asHtml = true
    ...
    var img =
      hasMeta && elt.firstChild != null
        ? elt.firstChild.nextSibling
        : elt.firstChild
