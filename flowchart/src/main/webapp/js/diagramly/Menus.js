...
;(function () {
  ...
  Menus.prototype.removeCustomFont = function (name, url) {
    for (var i = 0; i < this.customFonts.length; i++) {
      if (this.customFonts[i].name == name && this.customFonts[i].url == url) {
    ...
    for (var i = 0; i < this.defaultFonts.length; i++) {
      ...
      if (
        (typeof value !== 'string' &&
          ...
          value.fontUrl == url) ||
        (typeof value === 'string' && value == name && url == null)
      ) {
        return true
      }
    }
  ...
  Menus.prototype.init = function () {
    ...
    editorUi.actions.put(
      'insertTemplate',
      new Action(mxResources.get('template') + '...', function () {
        if (
          graph.isEnabled() &&
    ...
    var shareCursorAction = editorUi.actions.addAction(
      'shareCursor',
      function () {
    ...
    shareCursorAction.setSelectedCallback(function () {
      return editorUi.isShareCursorPosition()
    })

    var showRemoteCursorsAction = editorUi.actions.addAction(
      'showRemoteCursors',
      function () {
    ...
    showRemoteCursorsAction.setSelectedCallback(function () {
      return editorUi.isShowRemoteCursors()
    })
    ...
    meterAction.setSelectedCallback(function () {
      return editorUi.editor.graph.view.unit == mxConstants.METERS
    })
    ...
    this.put(
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          this.addMenuItems(
            menu,
            ['points', 'inches', 'millimeters', 'meters'],
            parent
          )
    ...
    pagesAction.setSelectedCallback(function () {
      return editorUi.isTabContainerVisible()
    })

    var rulerAction = editorUi.actions.addAction('ruler', function () {
      editorUi.setRulerVisible(!editorUi.isRulerVisible())
    })
    rulerAction.setEnabled(
      editorUi.canvasSupported && document.documentMode != 9
    )
    rulerAction.setToggleAction(true)
    rulerAction.setSelectedCallback(function () {
      return editorUi.isRulerVisible()
    })

    var fullscreenAction = editorUi.actions.addAction(
      'fullscreen',
      function () {
    ...
    var lightModeAction = editorUi.actions.put(
      'lightMode',
      new Action(mxResources.get('light'), function (e) {
    ...
    lightModeAction.setSelectedCallback(function () {
      return (
        !editorUi.isAutoDarkMode(true) &&
        !Editor.isDarkMode() &&
        !Editor.cssDarkMode
      )
    })

    var darkModeAction = editorUi.actions.put(
      'darkMode',
      new Action(mxResources.get('dark'), function (e) {
    ...
    darkModeAction.setSelectedCallback(function () {
      return (
        !editorUi.isAutoDarkMode(true) &&
        (Editor.isDarkMode() || Editor.cssDarkMode)
      )
    })

    var autoModeAction = editorUi.actions.put(
      'autoMode',
      new Action(mxResources.get('automatic'), function (e) {
    ...
    if (mxUtils.indexOf(Editor.themes, 'simple') >= 0) {
      var toggleSimpleModeAction = editorUi.actions.put(
        'toggleSimpleMode',
        new Action(mxResources.get('simple'), function (e) {
      ...
      toggleSimpleModeAction.visible =
        Editor.currentTheme != 'min' &&
        Editor.currentTheme != 'sketch' &&
        Editor.currentTheme != 'atlas'
      toggleSimpleModeAction.setSelectedCallback(function () {
        return Editor.currentTheme == 'simple'
      })
    }

    var toggleSketchModeAction = editorUi.actions.put(
      'toggleSketchMode',
      new Action(mxResources.get('sketch'), function (e) {
        editorUi.setSketchMode(!Editor.sketchMode)
      })
    )

    toggleSketchModeAction.setToggleAction(true)
    toggleSketchModeAction.setSelectedCallback(function () {
        ...
        new Action(
          ...
          function () {
            if (graph.isEnabled()) {
              ...
              editorUi.freehandWindow.window.setVisible(
    ...
    editorUi.actions.put(
      'exportXml',
      new Action(mxResources.get('formatXml') + '...', function () {
        ...
        var selection = editorUi.addCheckbox(
          div,
          mxResources.get('selectionOnly'),
          false,
          graph.isSelectionEmpty()
        )
        var compressed = editorUi.addCheckbox(
          div,
          mxResources.get('compressed'),
          Editor.defaultCompressed
        )
        var pages = editorUi.addCheckbox(
          div,
    ...
    editorUi.actions.put(
      'exportHtml',
      new Action(mxResources.get('formatHtmlEmbedded') + '...', function () {
        if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
          editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
            editorUi.spinner.stop()
              ...
              function (
                ...
                editorUi.createHtml(
                  ...
                  mxUtils.bind(this, function (html, scriptTag) {
                    ...
                    editorUi.saveData(
                      basename +
                        (basename.substring(basename.lenth - 7) == '.drawio'
                          ? ''
                          : '.drawio') +
                        '.html',
                      'html',
    ...
    editorUi.actions.put(
      'exportPdf',
      new Action(mxResources.get('formatPdf') + '...', function () {
        if (
          ...
          editorUi.showDialog(
            new PrintDialog(editorUi, mxResources.get('formatPdf')).container,
            320,
            editorUi.pages != null &&
              editorUi.pages.length > 1 &&
              (editorUi.editor.editable || urlParams['hide-pages'] != '1')
              ? 350
              : 280,
            true,
            true
          ...
          editorUi.showPrintDialog(
            mxResources.get('formatPdf'),
            mxUtils.bind(this, function (preview, args) {
              ...
              var pageRange =
                !args.allPages &&
                (args.pagesFrom != currentPage || args.pagesTo != currentPage)
                  ? {
                      from: Math.max(
                        0,
                        Math.min(pageCount - 1, args.pagesFrom - 1)
                      ),
                      to: Math.max(0, Math.min(pageCount - 1, args.pagesTo - 1))
                    }
                  : null
              ...
              editorUi.downloadFile(
                ...
                noPages ||
                  (!args.allPages &&
                    args.pagesFrom == currentPage &&
                    args.pagesTo == currentPage),
                !args.crop,
                args.transparent,
                args.scale,
                null,
      ...
      mxUtils.bind(this, function () {
        ...
        if (graph.getSelectionCount() == 1) {
          ...
          if (
            ...
            var dlg = new EditShapeDialog(
              editorUi,
              cell,
              mxResources.get('editShape') + ':',
              630,
              400
            )
    ...
    editorUi.actions.addAction('revisionHistory...', function () {
      if (!editorUi.isRevisionHistorySupported()) {
        editorUi.showError(
          ...
          mxResources.get('notAvailable'),
          mxResources.get('ok')
        )
      } else if (
        editorUi.spinner.spin(document.body, mxResources.get('loading'))
      ) {
        editorUi.getRevisions(
          mxUtils.bind(this, function (revs, restoreFn) {
            editorUi.spinner.stop()
          ...
          mxUtils.bind(this, function (err) {
            editorUi.handleError(
              err != null ? err : mxResources.get('notAvailable')
            )
          })
        )
      }
    ...
    var action = editorUi.actions.addAction(
      'synchronize',
      function () {
    ...
    editorUi.actions.addAction('upload...', function () {
      ...
      if (file != null) {
        ...
        var filename =
          file.getTitle() != null ? file.getTitle() : editorUi.defaultFilename
        editorUi.openLink(
    ...
    editorUi.actions.addAction(
      ...
      function () {
        if (
          graph.isEnabled() &&
          !graph.isSelectionEmpty() &&
          editorUi.copiedStyle != null
        ) {
          graph.pasteCellStyles(
            graph.includeDescendants(graph.getSelectionCells()),
            editorUi.copiedStyle,
            editorUi.copiedStyle,
            true
          )
    ...
    editorUi.actions.put(
      'exportSvg',
      new Action(mxResources.get('formatSvg') + '...', function () {
        editorUi.showExportDialog(
          ...
          true,
          editorUi.lastExportSvgEditable,
          'svg',
          true
        )
      })
    ...
    function exportImage(format, defaultEditable, done) {
      if (editorUi.isExportToCanvas()) {
        ...
        editorUi.showRemoteExportDialog(
          mxResources.get('export'),
          null,
          mxUtils.bind(
            this,
            function (ignoreSelection, editable, transparent, scale, border) {
              editorUi.downloadFile(
                editable && format == 'png' ? 'xmlpng' : format,
                null,
                null,
                ignoreSelection,
                null,
    ...
    editorUi.actions.put(
      'exportPng',
      new Action(mxResources.get('formatPng') + '...', function () {
        exportImage(
          'png',
          editorUi.lastExportPngEditable,
          function (
            scale,
            transparentBackground,
            ignoreSelection,
    ...
    editorUi.actions.put(
      'exportJpg',
      new Action(mxResources.get('formatJpg') + '...', function () {
        exportImage('jpeg')
      })
    )
    ...
    editorUi.actions.put(
      'exportWebp',
      new Action(mxResources.get('formatWebp') + '...', function () {
        exportImage('webp')
      })
    )
    ...
    action = editorUi.actions.addAction(
      ...
      mxUtils.bind(this, function () {
        ...
        var xml = mxUtils.getXml(
          cells.length == 0
            ? editorUi.editor.getGraphXml()
    ...
    editorUi.actions.put(
      'about',
      new Action('v' + EditorUi.VERSION, function (arg1, evt) {
        // do nothing
      })
    )
    ...
    editorUi.actions.addAction('support...', function () {
      if (EditorUi.isElectronApp) {
        editorUi.openLink(
          'https://github.com/jgraph/drawio-desktop/wiki/Getting-Support'
        )
      } else {
        editorUi.openLink(
    ...
    action = editorUi.actions.addAction(
      ...
      mxUtils.bind(this, function () {
        if (this.tagsWindow == null) {
          this.tagsWindow = new TagsWindow(
            editorUi,
          ...
          this.tagsWindow.window.addListener(
            'show',
            mxUtils.bind(this, function () {
    ...
    action.setSelectedCallback(
      mxUtils.bind(this, function () {
        return this.tagsWindow != null && this.tagsWindow.window.isVisible()
      })
    )

    if (
      Editor.gptApiKey != null &&
      Editor.gptModel != null &&
      Editor.gptUrl != null &&
      !editorUi.isOffline() &&
      !EditorUi.isElectronApp
    ) {
        ...
        mxUtils.bind(this, function () {
          if (this.chatWindow == null) {
            this.chatWindow = new ChatWindow(
              editorUi,
              220,
              document.body.offsetHeight - 380,
              250,
              320
            )
            ...
            this.chatWindow.window.setVisible(
              !this.chatWindow.window.isVisible()
            )
          }
        })
      )
      ...
      action.setSelectedCallback(
        mxUtils.bind(this, function () {
          return this.chatWindow != null && this.chatWindow.window.isVisible()
        })
      )
    }
    ...
    action = editorUi.actions.addAction(
      'findReplace',
      mxUtils.bind(this, function (arg1, evt) {
        var findReplace =
          graph.isEnabled() && (evt == null || !mxEvent.isShiftDown(evt))
    ...
    editorUi.actions.put(
      'exportVsdx',
      new Action(mxResources.get('formatVsdx') + ' (beta)...', function () {
        var noPages = editorUi.pages == null || editorUi.pages.length <= 1
      ...
      editorUi.actions.addAction('configuration...', function () {
        ...
        if (
          ...
          buttons.push([
            ...
            function () {
              editorUi.showLocalStorageDialog(
                mxResources.get('preferences') + ':',
                Editor.settingsKey,
                [
                  [
        ...
        editorUi.showLocalStorageDialog(
          mxResources.get('configuration') + ':',
          Editor.configurationKey,
          buttons,
          splashCb.parentNode,
    ...
    if (mxClient.IS_CHROMEAPP || isLocalStorage) {
        ...
        new Menu(
          mxUtils.bind(this, function (menu, parent) {
            ...
            var addLangItem = mxUtils.bind(this, function (id) {
              var lang =
                id == '' ? mxResources.get('automatic') : mxLanguageMap[id]
              var item = null
      ...
      Menus.prototype.createMenubar = function (container) {
        ...
        if (
          (urlParams['embed'] != '1' || urlParams['atlas'] == '1') &&
          menubar != null &&
              ...
              if (Editor.currentTheme == 'simple') {
                if (iw < 750) {
                  elt.style.backgroundImage =
                    'url(' + Editor.thinDoubleArrowRightImage + ')'
                } else {
                  elt.style.backgroundImage =
                    'url(' +
                    (Editor.isDarkMode() || Editor.cssDarkMode
    ...
    layoutMenu.funct = function (menu, parent) {
      ...
      menu.addItem(
        mxResources.get('orgChart'),
        null,
        function () {
          var branchOptimizer = null,
            parentChildSpacingVal = 20,
            siblingSpacingVal = 20

          // Invoked when orgchart code was loaded
          var delayed = function () {
            if (
              typeof mxOrgChartLayout !== 'undefined' &&
              branchOptimizer != null
            ) {
          ...
          var customBtn = mxUtils.button(
            ...
            function () {
              ...
              editorUi.hideDialog()
              editorUi.showCustomLayoutDialog(JSON.stringify(value, null, 2))
            }
          )
              ...
              mxUtils.bind(this, function (e) {
                ...
                if (e.keyCode == 13 && term.length > 0) {
                  this.editorUi.openLink(
                    'https://www.drawio.com/search?src=' +
                      (EditorUi.isElectronApp
                        ? 'DESKTOP'
    ...
    if (urlParams['test'] == '1') {
      ...
      editorUi.actions.addAction(
        ...
        mxUtils.bind(this, function () {
          if (!graph.isSelectionEmpty()) {
            ...
            editorUi.showTextDialog(
              'Create Sidebar Entry',
              "this.addDataEntry('tag1 tag2', " +
                bbox.width +
                ', ' +
                bbox.height +
                ", 'The Title', '" +
        ...
        mxUtils.bind(this, function () {
          ...
          function checkBounds(shape) {
            return (
              ...
              (!isNaN(shape.boundingBox.x) &&
                !isNaN(shape.boundingBox.y) &&
                !isNaN(shape.boundingBox.width) &&
      ...
      editorUi.actions.addAction(
        ...
        mxUtils.bind(this, function () {
          var xml =
            editorUi.pages != null && editorUi.getCurrentFile() != null
              ? editorUi
                  .getCurrentFile()
                  .getAnonymizedXmlForPages(editorUi.pages)
              : ''

          var dlg = new TextareaDialog(editorUi, 'Paste Data:', xml, function (
            newValue
          ) {
            if (newValue.length > 0) {
              try {
                ...
                var pages = editorUi.getPagesForNode(
                  doc.documentElement,
                  'mxGraphModel'
                )
                ...
                function checkModel(node) {
                  ...
                  if (root == null) {
                    ...
                    if (
                      Object.keys(visited).length != Object.keys(allIds).length
                    ) {
              ...
              [
                ...
                function (evt, input) {
                  ...
                  dlg.textarea.value =
                    ...
                    ' Checksum ' +
                    editorUi.getHashValueForPages(snapshot)
                }
              ],
              [
                'Diff',
                function (evt, input) {
                  try {
                    dlg.textarea.value = JSON.stringify(
                      editorUi.diffPages(snapshot, editorUi.pages),
                      null,
                      2
                    )
                  } catch (e) {
            ...
            if (
              ...
              file.getLatestVersion(
                function (latestFile) {
                  ...
                  var localChecksum = editorUi.getHashValueForPages(
                    editorUi.pages
                  )
      ...
      editorUi.actions.addAction(
        ...
        mxUtils.bind(this, function () {
          ...
          if (file != null && file.isRealtime()) {
            console.log(
              'Checksum ownPages',
              editorUi.getHashValueForPages(file.ownPages)
            )
            console.log(
              'Checksum theirPages',
              editorUi.getHashValueForPages(file.theirPages)
            )
            console.log(
              'diff ownPages/theirPages',
              editorUi.diffPages(file.ownPages, file.theirPages)
            )
            ...
            if (file.sync != null && file.sync.snapshot != null) {
              ...
              console.log(
                'diff ownPages/snapshot',
                editorUi.diffPages(file.ownPages, file.sync.snapshot)
              )
              console.log(
                'diff theirPages/snapshot',
                editorUi.diffPages(file.theirPages, file.sync.snapshot)
              )

              if (editorUi.pages != null) {
                console.log(
                  'diff snapshot/actualPages',
                  editorUi.diffPages(file.sync.snapshot, editorUi.pages)
                )
              }
            }
            ...
            if (editorUi.pages != null) {
              console.log(
                'diff ownPages/actualPages',
                editorUi.diffPages(file.ownPages, editorUi.pages)
              )
              console.log(
                'diff theirPages/actualPages',
                editorUi.diffPages(file.theirPages, editorUi.pages)
              )
            }
          }
          ...
          if (file != null) {
            console.log('Shadow pages', [
              editorUi.getXmlForPages(file.getShadowPages())
            ])
          }

          if (editorUi.pages != null) {
            console.log(
              'Checksum actualPages',
      ...
      editorUi.actions.addAction(
        ...
        mxUtils.bind(this, function () {
          ...
          if (file != null && file.isRealtime() && file.shadowPages != null) {
            console.log(
              'patching actualPages to shadowPages',
              file.patch([editorUi.diffPages(file.shadowPages, editorUi.pages)])
            )
        ...
        mxUtils.bind(this, function () {
          if (prevRevalidate != null) {
            ...
            graph.view.revalidate = function () {
              ...
              EditorUi.debug(
                '[Performance] mxGraphView.revalidate',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevSelectPage != null) {
            ...
            editorUi.selectPage = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.selectPage',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevDiffPages != null) {
            ...
            editorUi.diffPages = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.diffPages',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevPatchPages != null) {
            ...
            editorUi.patchPages = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.patchPages',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevClonePages != null) {
            ...
            editorUi.clonePages = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.clonePages',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevGetHashValueForPages != null) {
            ...
            editorUi.getHashValueForPages = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.getHashValueForPages',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevResolveCrossReferences != null) {
            ...
            editorUi.resolveCrossReferences = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.resolveCrossReferences',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          if (prevGetFileData != null) {
            ...
            editorUi.getFileData = function () {
              ...
              EditorUi.debug(
                '[Performance] EditorUi.getFileData',
                [this],
                'time',
                Date.now() - t0 + ' ms',
                'args',
          ...
          EditorUi.debug(
            '[Performance]',
            prevRevalidate != null ? 'Enabled' : 'Disabled'
          )
        })
      )
      ...
      this.put(
        ...
        new Menu(
          mxUtils.bind(this, function (menu, parent) {
            ...
            var item = menu.addItem(
              mxResources.get('testPerformance'),
              null,
              function () {
    ...
    editorUi.actions.put(
      'shapes',
      new Action(mxResources.get('moreShapes') + '...', function (evt) {
        if (mxClient.IS_CHROMEAPP || !editorUi.isOffline()) {
          editorUi.showDialog(
    ...
    editorUi.actions.put(
      'createShape',
      new Action(mxResources.get('shape') + '...', function (evt) {
        if (graph.isEnabled()) {
          var cell = new mxCell(
            '',
    ...
    editorUi.actions.put(
      'embedHtml',
      new Action(mxResources.get('html') + '...', function () {
        if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
          editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
            editorUi.spinner.stop()
                    ...
                    var dlg = new EmbedDialog(
                      ...
                      function () {
                        ...
                        if (doc != null) {
                          ...
                          doc.writeln(
                            '<head><title>' +
                              encodeURIComponent(mxResources.get('preview')) +
                          ...
                          if (!direct) {
                            ...
                            wnd.document.body.insertBefore(
                              info,
                              wnd.document.body.firstChild
                            )

                            window.setTimeout(function () {
                              var script = document.createElement('script')
                              script.type = 'text/javascript'
                              script.src = /<script.*?src="(.*?)"/.exec(
                                scriptTag
                              )[1]
    ...
    editorUi.actions.put(
      'liveImage',
      new Action('Live image...', function () {
          ...
          editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
            ...
            if (url != null) {
              var dlg = new EmbedDialog(
                editorUi,
                '<img src="' +
                  (current.constructor != DriveFile
                    ? url
                    : 'https://drive.google.com/uc?id=' + current.getId()) +
                  '"/>'
              )
              editorUi.showDialog(dlg.container, 450, 240, true, true)
              dlg.init()
    ...
    editorUi.actions.put(
      ...
      new Action(mxResources.get('image') + '...', function () {
        editorUi.showEmbedImageDialog(
          function (fit, shadow, retina, lightbox, editLink, layers) {
            if (
              editorUi.spinner.spin(document.body, mxResources.get('loading'))
            ) {
              editorUi.createEmbedImage(
                fit,
                shadow,
                retina,
                lightbox,
    ...
    editorUi.actions.put(
      ...
      new Action(mxResources.get('formatSvg') + '...', function () {
        editorUi.showEmbedImageDialog(
          function (fit, shadow, image, lightbox, editLink, layers) {
            if (
              editorUi.spinner.spin(document.body, mxResources.get('loading'))
            ) {
              editorUi.createEmbedSvg(
                fit,
                shadow,
                image,
                lightbox,
          ...
          mxResources.get('image'),
          true,
          'https://www.drawio.com/doc/faq/embed-svg.html'
        )
      })
    )
    ...
    editorUi.actions.put(
      'embedIframe',
      new Action(mxResources.get('iframe') + '...', function () {
        var bounds = graph.getGraphBounds()
    ...
    editorUi.actions.put(
      'embedNotion',
      new Action(mxResources.get('notion') + '...', function () {
        ...
        editorUi.showPublishLinkDialog(
          ...
          true,
          'https://www.drawio.com/blog/drawio-notion',
          footer
        )
      })
    )
    ...
    if (isLocalStorage || mxClient.IS_CHROMEAPP) {
      ...
      if (urlParams['plugins'] != '0') {
        editorUi.actions.addAction('plugins...', function () {
          editorUi.showDialog(
            new PluginsDialog(editorUi).container,
            380,
            240,
            true,
            false
    ...
    if (
      window.matchMedia &&
      document.getElementById('high-contrast-stylesheet') != null
    ) {
    ...
    if (urlParams['embed'] == '1') {
      ...
      var saveAndExitAction = editorUi.actions.addAction(
        'saveAndExit',
        function () {
      ...
      editorUi.actions.addAction(
        ...
        null,
        null,
        urlParams['embedInline'] == '1' ? 'Escape' : null
      )
    }
    ...
    this.put(
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          var doImportFile = mxUtils.bind(
            this,
            function (data, mime, filename) {
              // Gets insert location
              var view = graph.view
              var bds = graph.getGraphBounds()
              var x = graph.snap(
                Math.ceil(
                  Math.max(0, bds.x / view.scale - view.translate.x) +
                    4 * graph.gridSize
                )
              )
              var y = graph.snap(
                Math.ceil(
                  Math.max(
                    0,
                    (bds.y + bds.height) / view.scale - view.translate.y
                  ) +
                    4 * graph.gridSize
                )
              )
                        ...
                        mxUtils.bind(this, function (data2, w2, h2) {
                          var s = resizeImages
                            ? Math.min(
                                1,
                                Math.min(
                                  editorUi.maxImageSize / w2,
                                  editorUi.maxImageSize / h2
                                )
                              )
                            : 1
            ...
            service.pickFile(function (id) {
              if (
                ...
                service.getFile(
                  ...
                  function (file) {
                    var mime =
                      file.getData().substring(0, 11) == 'data:image/'
                        ? getMimeType(file.getTitle())
                        : 'text/xml'

                    // Imports SVG as images
                    if (
                      /\.svg$/i.test(file.getTitle()) &&
                      !editorUi.editor.isDataSvg(file.getData())
                    ) {
                  ...
                  function (resp) {
                    editorUi.handleError(
                      resp,
                      resp != null ? mxResources.get('errorLoadingFile') : null
                    )
                  },
                ...
                var dlg = new FilenameDialog(
                  ...
                  function (fileUrl) {
                    if (
                      fileUrl != null &&
                      fileUrl.length > 0 &&
                      editorUi.spinner.spin(
                        document.body,
                        mxResources.get('loading')
                      )
                    ) {
                      var mime = /(\.png)($|\?)/i.test(fileUrl)
                        ? 'image/png'
                        : 'text/xml'

                      // Uses proxy to avoid CORS issues
                      editorUi.editor.loadUrl(
            ...
            if (item != null) {
              item.setAttribute(
                ...
                mxResources.get('automatic') +
                  ' (' +
                  mxResources.get(
                    Editor.cssDarkMode || Editor.isDarkMode() ? 'dark' : 'light'
                  ) +
                  ')'
              )
            }
    ...
    editorUi.actions.addAction('addToScratchpad', function (evt) {
      if (
        !graph.isSelectionEmpty() &&
        editorUi.addSelectionToScratchpad != null
      ) {
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          ...
          item = menu.addItem(
            ...
            function () {
              editorUi.setCurrentTheme(
                !Editor.isDarkMode() ? 'kennedy' : 'dark'
              )
            },
            parent
          )
          ...
          for (var i = 0; i < Editor.themes.length; i++) {
            mxUtils.bind(this, function (key) {
              item = menu.addItem(
                mxResources.get(key == 'min' ? 'minimal' : key),
                null,
                function () {
      ...
      mxUtils.bind(this, function () {
        ...
        if (file != null) {
          if (file.constructor == LocalFile && file.fileHandle != null) {
            editorUi.showSaveFilePicker(
              mxUtils.bind(editorUi, function (fileHandle, desc) {
              ...
              null,
              editorUi.createFileSystemOptions(file.getTitle())
            )
          } else {
            var filename =
              file.getTitle() != null
            ...
            var dlg = new FilenameDialog(
              ...
              mxUtils.bind(this, function (title) {
                if (
                  title != null &&
                  title.length > 0 &&
                  file != null &&
              ...
              null,
              FilenameDialog.filenameHelpLink,
              null,
              null,
        ...
        if (file != null) {
          ...
          if (file.constructor == DriveFile) {
            var dlg = new CreateDialog(
              editorUi,
              title,
              mxUtils.bind(this, function (newTitle, mode) {
                if (mode == '_blank') {
    ...
    editorUi.actions.put(
      'openFolder',
      new Action(
        mxResources.get('openIt', [mxResources.get('folder')]) + '...',
    ...
    editorUi.actions.addAction(
      ...
      mxUtils.bind(this, function () {
        ...
        if (
          file.getMode() == App.MODE_GOOGLE ||
          file.getMode() == App.MODE_ONEDRIVE
        ) {
          var isInRoot = false
          ...
          editorUi.pickFolder(
            file.getMode(),
            mxUtils.bind(this, function (folderId) {
              if (
                editorUi.spinner.spin(document.body, mxResources.get('moving'))
              ) {
                file.move(
                  folderId,
                  mxUtils.bind(this, function (resp) {
    ...
    editorUi.actions.put(
      'useOffline',
      new Action(mxResources.get('useOffline') + '...', function () {
        editorUi.openLink('https://app.draw.io/')
      })
    )

    this.editorUi.actions.addAction(
      'share...',
      mxUtils.bind(this, function () {
        try {
          var file = editorUi.getCurrentFile()
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          ...
          if (
            file != null &&
            (file.getMode() == App.MODE_GOOGLE ||
              file.getMode() == App.MODE_GITHUB) &&
            /(\.png)$/i.test(file.getTitle())
          ) {
                ...
                function (cancel, isEsc) {
                  if (isEsc) {
                    ...
                    if (dlg.graph.container.parentNode != null) {
                      dlg.graph.container.parentNode.removeChild(
                        dlg.graph.container
                      )
                    }
    ...
    var insertEdge = function (value, length, style, pt) {
      ...
      cell.geometry.setTerminalPoint(
        new mxPoint(pt.x + cell.geometry.width, pt.y),
        false
      )
    ...
    var toggleShapes = editorUi.actions.put(
      'toggleShapes',
      new Action(
        ...
        null,
        null,
        Editor.ctrlKey + '+Shift+K'
      )
    )

    toggleShapes.setToggleAction(true)
    toggleShapes.setSelectedCallback(
      mxUtils.bind(this, function () {
        return (
          (editorUi.sidebarWindow != null &&
            editorUi.sidebarWindow.window.isVisible()) ||
    ...
    editorUi.addInsertMenuItems = mxUtils.bind(
      this,
      function (menu, parent, methods) {
        for (var i = 0; i < methods.length; i++) {
          if (methods[i] == '-') {
            menu.addSeparator(parent)
          } else {
            editorUi.addInsertItem(
              menu,
              parent,
    ...
    this.put(
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          editorUi.addInsertMenuItems(menu, parent, [
            'horizontalFlow',
            'verticalFlow',
            '-',
            'horizontalTree',
      ...
      this.put(
        ...
        new Menu(function (menu, parent) {
          ...
          if (urlParams['confLib'] == '1') {
            ...
            menu.addItem(
              mxResources.get('confluenceCloud') + '...',
              null,
              function () {
    ...
    var action = editorUi.actions.addAction(
      ...
      mxUtils.bind(this, function () {
        if (this.commentsWindow == null) {
          ...
          this.commentsWindow = new CommentsWindow(
            editorUi,
    ...
    action.setSelectedCallback(
      mxUtils.bind(this, function () {
        return (
          this.commentsWindow != null && this.commentsWindow.window.isVisible()
        )
      })
    )

    // Destroys comments window to force update or disable if not supported
    editorUi.editor.addListener(
      'fileLoaded',
      mxUtils.bind(this, function () {
        if (this.commentsWindow != null) {
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          if (Editor.currentTheme == 'simple') {
            ...
            this.addMenuItems(
              menu,
              ['format', 'outline', 'layers', 'tags', 'chatWindowTitle'].concat(
                editorUi.commentsSupported() ? ['comments', '-'] : ['-']
              )
            )
            ...
            menu.addItem(
              ...
              mxUtils.bind(this, function () {
                editorUi.duplicatePage(
                  page,
                  mxResources.get('copyOf', [page.getName()])
                )
              }),
              parent
            )
            ...
            if (urlParams['embed'] != 1) {
              ...
              if (
                ...
                menu.addItem(
                  ...
                  null,
                  mxUtils.bind(this, function () {
                    editorUi.editor.editAsNew(
                      editorUi.getFileData(true, null, null, null, true, true)
                    )
                  }),
                  parent
                )
          ...
          if (editorUi.pages != null) {
            for (var i = 0; i < editorUi.pages.length; i++) {
              mxUtils.bind(this, function (index) {
                ...
                if (
                  editorUi.pages[index] == page &&
                  !editorUi.editor.graph.isLightboxView()
                ) {
                  item = editorUi.menus.addSubmenu(
                    'currentPage',
                    menu,
                    parent,
                  ...
                  item = menu.addItem(
                    editorUi.getShortPageName(editorUi.pages[index]),
                    null,
                    mxUtils.bind(this, function () {
                ...
                item.setAttribute(
                  'title',
                  editorUi.pages[index].getName() +
                    ' (' +
                    (index + 1) +
                    '/' +
                    editorUi.pages.length +
                    ')' +
                    (id != null ? ' [' + id + ']' : '')
                )
    ...
    if (EditorUi.isElectronApp) {
      ...
      var spellCheckAction = editorUi.actions.addAction(
        'spellCheck',
        function () {
      ...
      spellCheckAction.setSelectedCallback(function () {
      ...
      storeBkpAction.setSelectedCallback(function () {
        return enableStoreBkp
      })

      var enableGoogleFonts = urlParams['isGoogleFontsEnabled'] == '1'

      var googleFontsAction = editorUi.actions.addAction(
        'googleFonts',
        function () {
      ...
      googleFontsAction.setSelectedCallback(function () {
        return enableGoogleFonts
      })
        ...
        mxUtils.bind(this, function (menu, parent) {
          ...
          if (
            ...
            if (
              ...
              editorUi.mode != App.MODE_ATLAS &&
              urlParams['embedInline'] != '1' &&
              (Editor.isDarkMode() || (!mxClient.IS_IE && !mxClient.IS_IE11))
            ) {
            ...
            if (EditorUi.isElectronApp) {
              this.addMenuItems(
                menu,
                ['googleFonts', 'spellCheck', 'autoBkp', 'drafts', '-'],
                parent
              )
            }
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          ...
          if (mxClient.IS_CHROMEAPP || EditorUi.isElectronApp) {
            editorUi.menus.addMenuItems(
              menu,
              ['open', '-', 'synchronize', 'properties', '-', 'saveAs', '-'],
              parent
            )
          ...
          if (!sketchTheme && Editor.currentTheme != 'min') {
            if (
              file != null &&
              editorUi.fileNode != null &&
              urlParams['embedInline'] != '1'
            ) {
              var filename =
                file.getTitle() != null
                  ? file.getTitle()
                  : editorUi.defaultFilename
    ...
    this.put(
      ...
      new Menu(
        mxUtils.bind(this, function (menu, parent) {
          ...
          if (
            file != null &&
            (file.constructor == DriveFile || file.constructor == OneDriveFile)
          ) {
            editorUi.menus.addMenuItems(
              menu,
              ['save', 'makeCopy', '-', 'rename', 'moveToFolder'],
              parent
            )
          } else {
        ...
        mxUtils.bind(this, function (menu, parent) {
          ...
          if (urlParams['embed'] == '1') {
            ...
            if (file != null && file.constructor == DriveFile) {
              this.addMenuItems(
                menu,
                ['rename', 'makeCopy', 'openFolder', 'moveToFolder'],
                parent
              )
            } else {
              if (
        ...
        mxUtils.bind(this, function (menu, parent) {
          var addItem = mxUtils.bind(
            ...
            function (fontName, fontUrl, deletable, fontLabel, tooltip) {
              ...
              var tr = this.styleChange(
                menu,
                fontLabel != null ? fontLabel : fontName,
                [mxConstants.STYLE_FONTFAMILY, 'fontSource', 'FType'],
                [
                  fontName,
                  fontUrl != null ? encodeURIComponent(fontUrl) : null,
                  null
                ],
                null,
                parent,
            ...
            if (typeof value === 'string') {
              ...
              reserved[
                encodeURIComponent(value.fontFamily) +
                  '@' +
                  encodeURIComponent(value.fontUrl)
              ] = true
          ...
          function addEntry(entry) {
            var key =
              ...
              (entry.url == null ? '' : '@' + encodeURIComponent(entry.url))
          ...
          if (entries.length > 0) {
            for (var i = 0; i < entries.length; i++) {
              addItem(entries[i].name, entries[i].url, true, entries[i].label)
            }
          ...
          menu.addItem(
            ...
            mxUtils.bind(this, function () {
              ...
              if (graph.isEditing()) {
                ...
                if (node != null) {
                  ...
                  if (css != null) {
                    ...
                    var state = graph
                      .getView()
                      .getState(graph.cellEditor.getEditingCell())
                    var curUrl =
                      state != null ? state.style['fontSource'] : null
                ...
                if (state != null) {
                  curFontName =
                    state.style[mxConstants.STYLE_FONTFAMILY] || curFontName
              ...
              if (
                curUrl != null &&
                curUrl.substring(0, PROXY_URL.length) == PROXY_URL
              ) {
                curUrl = decodeURIComponent(
                  curUrl.substr((PROXY_URL + '?url=').length)
                )
              }
              ...
              var dlg = new FontDialog(
                ...
                curUrl,
                curType,
                mxUtils.bind(this, function (fontName, fontUrl, type) {
              ...
              this.editorUi.showDialog(
                dlg.container,
                380,
                130 +
                  (Editor.enableWebFonts ? 70 : 0) +
                  (urlParams['isGoogleFontsEnabled'] != '0' ? 50 : 0),
                true,
                true
              )
              dlg.init()
