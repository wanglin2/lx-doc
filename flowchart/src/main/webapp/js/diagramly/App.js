  ...
  EditorUi.call(
    ...
    lightbox != null
      ...
      : urlParams['lightbox'] == '1' ||
          (uiTheme == 'min' && urlParams['chrome'] != '0')
  )
      ...
      if (file != null && file.isModified()) {
        var evt = {
          ...
          action:
            ...
            (file.savingFile && file.savingFileTime != null
              ? '_' +
            ...
            '-age_' +
            (file.ageStart != null
              ? Math.round((Date.now() - file.ageStart.getTime()) / 1000)
              : 'x') +
            (this.editor.autosave ? '' : '-nosave') +
            (file.isAutosave() ? '' : '-noauto') +
            '-open_' +
            (file.opened != null
              ? Math.round((Date.now() - file.opened.getTime()) / 1000)
              : 'x') +
            '-save_' +
            (file.lastSaved != null
              ? Math.round((Date.now() - file.lastSaved.getTime()) / 1000)
              : 'x') +
            '-change_' +
            (file.lastChanged != null
              ? Math.round((Date.now() - file.lastChanged.getTime()) / 1000)
              : 'x') +
            '-alive_' +
            Math.round((Date.now() - App.startTime.getTime()) / 1000),
        ...
        if (
          ...
          evt.label +=
            (this.drive.user != null
              ? '-user_' + this.drive.user.id
              : '-nouser') +
            '-rev_' +
  ...
  window.geOpenWindow = mxUtils.bind(this, function (url, pre, fallback) {
    if (urlParams['openInSameWin'] == '1' || navigator.standalone) {
      ...
      if (wnd == null || wnd === undefined) {
        this.showDialog(
          new PopupDialog(this, url, pre, fallback).container,
          320,
          160,
          true,
          true
      ...
      if (urlParams['embed'] != '1') {
        if (typeof window.DriveClient === 'function') {
          if (
            ...
            if (
              App.mode == App.MODE_GOOGLE ||
              (urlParams['state'] != null && window.location.hash == '') ||
            ...
            // Keeps lazy loading for fallback to authenticated Google file if not public in loadFile
              ...
              urlParams['chrome'] == '0' &&
              (window.location.hash == null ||
                window.location.hash.substring(0, 45) !==
...
App.isSameDomain = function (link) {
  ...
  return (
    a.protocol === window.location.protocol || a.host === window.location.host
  )
}
      ...
      mxUtils.getAll(
        ...
        function (xhr) {
          ...
          if (urlParams['dev'] == '1' || EditorUi.isElectronApp) {
            //TODO check if we can remove these scripts loading from index.html
            realMain()
          } else {
...
App.prototype.chevronDownImage =
 ...
 *
 */
App.prototype.compactMode = true
...
App.prototype.init = function () {
  ...
  try {
    this.gitHub =
      (!mxClient.IS_IE ||
        ...
        mxClient.IS_EDGE) &&
      urlParams['gh'] != '0' &&
      (urlParams['embed'] != '1' || urlParams['gh'] == '1')
        ? new GitHubClient(this)
        : null

    if (this.gitHub != null) {
      this.gitHub.addListener(
        'userChanged',
        mxUtils.bind(this, function () {
  ...
  try {
    this.gitLab =
      (!mxClient.IS_IE ||
        ...
        mxClient.IS_EDGE) &&
      urlParams['gl'] != '0' &&
      (urlParams['embed'] != '1' || urlParams['gl'] == '1')
        ? new GitLabClient(this)
        : null

    if (this.gitLab != null) {
      this.gitLab.addListener(
        'userChanged',
        mxUtils.bind(this, function () {
          this.updateUserElement()
          this.restoreLibraries()
    ...
    var initOneDriveClient = mxUtils.bind(this, function () {
      if (typeof OneDrive !== 'undefined') {
        try {
          ...
          this.fireEvent(
            new mxEventObject('clientLoaded', 'client', this.oneDrive)
          )
        } catch (e) {
          if (window.console != null) {
    ...
    var initTrelloClient = mxUtils.bind(this, function () {
      if (typeof window.Trello !== 'undefined') {
        try {
          ...
          // Notifies listeners of new client
          this.fireEvent(
            new mxEventObject('clientLoaded', 'client', this.trello)
          )
        } catch (e) {
          if (window.console != null) {
      ...
      if (typeof gapi !== 'undefined') {
        var doInit = mxUtils.bind(this, function () {
          try {
            ...
            // Notifies listeners of new client
            this.fireEvent(
              new mxEventObject('clientLoaded', 'client', this.drive)
            )
          } catch (e) {
            if (window.console != null) {
        ...
        if (window.DrawGapiClientCallback != null) {
          gapi.load(
            (urlParams['picker'] != '0' ? 'picker,' : '') + App.GOOGLE_APIS,
            doInit
          )
    ...
    var initDropboxClient = mxUtils.bind(this, function () {
      if (typeof Dropbox === 'function') {
        ...
        try {
          ...
          // Notifies listeners of new client
          this.fireEvent(
            new mxEventObject('clientLoaded', 'client', this.dropbox)
          )
        } catch (e) {
          if (window.console != null) {
    ...
    if (
      ...
      window.addEventListener(
        ...
        mxUtils.bind(this, function (e) {
          this.showBanner(
            'AddToHomeScreenFooter',
            mxResources.get('installApp'),
            function () {
              e.prompt()
            }
          )
      ...
      this.editor.addListener(
        ...
        mxUtils.bind(this, function () {
          ...
          if (
            ...
            urlParams['open'] == null &&
            urlParams['extAuth'] != '1' &&
            (mode == App.MODE_DEVICE || mode == App.MODE_BROWSER)
          ) {
  ...
  if (this.menubar != null) {
    ...
    if (Editor.currentTheme == 'atlas' || urlParams['atlas'] == '1') {
      ...
      this.menubar.container.insertBefore(
        this.icon,
        this.menubar.container.firstChild
      )
    }
  }
  ...
  try {
    if (
      ...
      for (var i = 0; i < window.location.ancestorOrigins.length; i++) {
        ...
        if (
          message.endsWith('.appsplus.co') ||
          message.endsWith(
            'confluence-cloud-excalidraw-ll3likebca-uc.a.run.app'
          )
        ) {
          return true
        }
...
App.prototype.scheduleSanityCheck = function () {
  if (
    ...
    this.sanityCheckThread = window.setTimeout(
      mxUtils.bind(this, function () {
        this.sanityCheckThread = null
        this.sanityCheck()
      }),
...
App.prototype.sanityCheck = function () {
  ...
  if (
    ...
    if (
      ...
      evt.label +=
        (this.drive.user != null ? '-user_' + this.drive.user.id : '-nouser') +
        '-rev_' +
        file.desc.headRevisionId +
        '-mod_' +
        file.desc.modifiedDate +
    ...
    this.showError(
      mxResources.get('unsavedChanges'),
      msg,
      mxResources.get('ignore'),
      mxUtils.bind(this, function () {
        this.hideDialog()
      }),
      null,
      ...
      mxUtils.bind(this, function () {
        ...
        this.actions
          .get(this.mode == null || !file.isEditable() ? 'saveAs' : 'save')
          .funct()
      }),
      null,
      null,
...
App.prototype.showRatingBanner = function () {
  if (
    ...
    star1.setAttribute(
      'style',
      'margin-top:-6px;cursor:pointer;margin-left:8px;'
    )
    ...
    star2.setAttribute(
      'style',
      'margin-top:-6px;margin-left:3px;cursor:pointer;'
    )
...
App.prototype.updateActionStates = function () {
  ...
  this.actions
    .get('revisionHistory')
    .setEnabled(this.isRevisionHistoryEnabled())
}
  ...
  if (isLocalStorage && localStorage != null) {
    ...
    if (recent == null) {
      ...
      for (var i = 0; i < recent.length; i++) {
        if (recent[i].mode == entry.mode && recent[i].id == entry.id) {
          recent.splice(i, 1)
        }
      }
    }
...
App.prototype.updateDocumentTitle = function () {
  ...
  if (
    file != null &&
    (this.editor.graph.isLightboxView() || Editor.currentTheme == 'simple') &&
    this.pages != null &&
...
App.prototype.getThumbnail = function (width, fn) {
  ...
  try {
    ...
    if (
      this.pages != null &&
      (darkTheme || this.currentPage != this.pages[0])
    ) {
        ...
        mxUtils.bind(this, function (canvas) {
          try {
            ...
            if (
              ...
              graph.container.parentNode != null
            ) {
  ...
  App.prototype.setMode = function (mode, remember) {
    ...
    if (this.appIcon != null) {
      ...
      if (mode == App.MODE_GOOGLE) {
        this.appIcon.setAttribute(
          'title',
          mxResources.get('openIt', [mxResources.get('googleDrive')])
        )
        this.appIcon.style.cursor = 'pointer'
      } else if (mode == App.MODE_DROPBOX) {
        this.appIcon.setAttribute(
          'title',
          mxResources.get('openIt', [mxResources.get('dropbox')])
        )
        this.appIcon.style.cursor = 'pointer'
      } else if (mode == App.MODE_ONEDRIVE) {
        this.appIcon.setAttribute(
          'title',
          mxResources.get('openIt', [mxResources.get('oneDrive')])
        )
        this.appIcon.style.cursor = 'pointer'
      } else {
        this.appIcon.removeAttribute('title')
        this.appIcon.style.cursor =
          mode == App.MODE_DEVICE ? 'pointer' : 'default'
      }
    }
    ...
    if (window.opener != null) {
      ...
      if (
        ...
        window.opener.openFile.setConsumer(
          mxUtils.bind(this, function (xml, filename, temp) {
            this.spinner.stop()
            ...
            var dot = !this.useCanvasForExport
    ...
    if (urlParams['embed'] != '1') {
      try {
        if (this.spinner.spin(document.body, mxResources.get('starting'))) {
          try {
            this.stateArg =
              urlParams['state'] != null && this.drive != null
...
App.prototype.showRefreshDialog = function (title, message) {
  if (!this.showingRefreshDialog) {
    ...
    this.showError(
      title || mxResources.get('externalChanges'),
            ...
            if (value != null) {
              ...
              if (value != null) {
                ...
                this.installMessageHandler(
                  mxUtils.bind(this, function (xml, evt) {
        ...
        var showCreateDialog = mxUtils.bind(this, function (xml) {
          ...
          if (urlParams['splash'] != '0') {
            ...
            var dlg = new CreateDialog(
              this,
              title,
              mxUtils.bind(this, function (filename, mode) {
                if (mode == null) {
                  this.hideDialog()
                  var prev = Editor.useLocalStorage
                  this.createFile(
                    filename.length > 0 ? filename : this.defaultFilename,
                    this.getFileData(),
                    null,
                    null,
                    null,
        ...
        if (
          ...
          if (this.stateArg.ids != null) {
            if (window.history && window.history.replaceState) {
              ...
              window.history.replaceState(
                null,
                null,
                window.location.pathname + this.getSearch(['state'])
              )
            }
          ...
          if (
            ...
            window.history.replaceState(
              null,
              null,
              window.location.pathname + this.getSearch(['open', 'sketch'])
            )
...
App.prototype.filterDrafts = function (filePath, guid, callback) {
  ...
  try {
    this.getDatabaseItems(
      mxUtils.bind(this, function (items) {
        EditorUi.debug('App.filterDrafts', [this], 'items', items)
...
App.prototype.addLanguageMenu = function (elt, addLabel, right) {
  ...
  if (langMenu != null) {
    ...
    img.className =
      Editor.currentTheme != 'atlas' ? 'geIcon geAdaptiveAsset' : ''
        ...
        reader.onload = mxUtils.bind(this, function (e) {
          try {
            if (success != null) {
              ...
              success(
                new LocalFile(this, data, file.name, null, fileHandle, file)
              )
            } else {
              this.openFileHandle(
                e.target.result,
...
App.prototype.createFileSystemOptions = function (name) {
  ...
  for (var i = 0; i < this.editor.diagramFileTypes.length; i++) {
    ...
    obj.accept[this.editor.diagramFileTypes[i].mimeType] = [
      '.' + this.editor.diagramFileTypes[i].extension
    ]
...
App.prototype.showSaveFilePicker = function (success, error, opts) {
  ...
  window.showSaveFilePicker(opts).then(
        ...
        window.openFile.setConsumer(
          mxUtils.bind(this, function (xml, filename) {
            var doOpenFile = mxUtils.bind(this, function () {
              ...
              if (dot) {
                filename =
                  filename.substring(0, filename.length - 4) + '.drawio'
              }
...
App.prototype.pickLibrary = function (mode) {
  ...
  var doLoadLibary = mxUtils.bind(this, function (file) {
    try {
      ...
      try {
        this.sidebar.palettes[file.getHash()][0].scrollIntoView({
          behavior: 'smooth'
        })
      } catch (e) {
        // ignore
          ...
          if (input.files != null) {
            for (var i = 0; i < input.files.length; i++) {
              mxUtils.bind(this, function (file) {
                ...
                reader.onload = mxUtils.bind(this, function (e) {
                  doLoadLibary(
                    new LocalLibrary(this, e.target.result, file.name)
                  )
                })
...
App.prototype.saveLibrary = function (
  ...
  try {
    ...
    var error = mxUtils.bind(this, function (resp) {
      ...
      this.handleError(
        resp,
        resp != null ? mxResources.get('errorSavingFile') : null
      )
    })
      ...
      this.pickFolder(
        ...
        mxUtils.bind(this, function (folderId) {
          if (
            ...
            this.drive.insertFile(
              name,
              xml,
              folderId,
              mxUtils.bind(this, function (newFile) {
                this.spinner.stop()
                this.hideDialog(true)
                this.libraryLoaded(newFile, images)
              }),
              error,
              this.drive.libraryMimeType
            )
          } else if (
            mode == App.MODE_GITHUB &&
            this.gitHub != null &&
...
App.prototype.loadTemplate = function (
  ...
  this.editor.loadUrl(
    realUrl,
    mxUtils.bind(this, function (responseData) {
      try {
        var data = !base64
          ? responseData
        ...
        if (isVisioFilename || this.isVisioData(data)) {
          ...
          this.importVisio(
            this.base64ToBlob(
              responseData.substring(responseData.indexOf(',') + 1)
            ),
            function (xml) {
              onload(xml)
            },
          ...
          this.isRemoteFileFormat(data, filterFn)
        ) {
...
App.prototype.isModeEnabled = function (mode) {
  if (mode == App.MODE_GOOGLE) {
    ...
    return (
      typeof window.TrelloClient === 'function' &&
      urlParams['tr'] == '1' &&
      mxClient.IS_SVG &&
      (document.documentMode == null || document.documentMode > 9)
    )
...
App.prototype.isModeReady = function (mode) {
  return (
    this.getServiceForName(mode) != null &&
    (mode != App.MODE_DROPBOX || typeof Dropbox.choose !== 'undefined')
  )
}
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
App.prototype.createFile = function (
  title,
  data,
  libs,
  mode,
  ...
  if (
    ...
    try {
      ...
      if (mode == App.MODE_GOOGLE && this.drive != null) {
        ...
        complete()
        this.fileCreated(
          new LocalFile(this, data, title, mode == null),
          libs,
          replace,
          done,
          clibs,
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
App.prototype.fileCreated = function (
  file,
  libs,
  replace,
  done,
...
App.prototype.loadFile = function (id, sameWindow, file, success, force) {
      ...
      if (id.charAt(0) == 'L') {
            ...
            this.handleError(
              ...
              mxUtils.bind(this, function () {
                ...
                window.location.hash =
                  tempFile != null ? tempFile.getHash() : ''
              })
            )
          })
        ...
        this.alert(
          ...
          mxUtils.bind(this, function () {
            window.location.href =
              'https://app.diagrams.net/?desc=' + id.substring(1)
          })
        )
        ...
        if (currentFile == null) {
          ...
          this.remoteInvoke(
            ...
            null,
            null,
            mxUtils.bind(this, function (data, desc) {
              this.spinner.stop()
        ...
        var doFallback = mxUtils.bind(this, function () {
          ...
          if (
            url.substring(0, 31) == 'https://drive.google.com/uc?id=' &&
        ...
        this.loadTemplate(
          url,
          mxUtils.bind(this, function (text) {
            this.spinner.stop()
          ...
          urlParams['template-filename'] != null
            ? decodeURIComponent(urlParams['template-filename'])
            : null
        )
      } else {
          ...
          if (peer == null) {
            this.handleError(
              ...
              mxUtils.bind(this, function () {
                var currentFile = this.getCurrentFile()
                window.location.hash =
                  currentFile != null ? currentFile.getHash() : ''
              })
            )
          } else {
                ...
                if (currentFile == null) {
                  ...
                  try {
                    ...
                    if (obj != null && !mxUtils.isEmptyObject(obj)) {
                      hash =
                        hash + '#' + encodeURIComponent(JSON.stringify(obj))
                    }
                  } catch (e) {
                    // ignore
                  }
  ...
  if (window.VueVm.fileData && window.VueVm.fileData.content) {
    const file = new LocalFile(
      this,
      window.VueVm.fileData.content,
      window.VueVm.fileData.name,
      this.mode
    )
    this.spinner.stop()
    this.fileLoaded(file)
    ...
    this.showDialog(
      new PopupDialog(this, this.getUrl() + '#' + id, null, fn).container,
      320,
      160,
      true,
      true
...
App.prototype.restoreLibraries = function () {
  ...
  function addLibs(libs) {
    for (var i = 0; i < libs.length; i++) {
      if (libs[i] != '' && mxUtils.indexOf(checked, libs[i]) < 0) {
        checked.push(libs[i])
      }
    }
  }
        ...
        mxUtils.bind(this, function (id, index) {
          if (
            ...
            if (service == 'L') {
              ...
              try {
                this.loadDesktopLib(
                  decodeURIComponent(id.substring(1)),
                  function (desktopLib) {
                    onload(desktopLib)
                  },
                  onerror
...
App.prototype.updateButtonContainer = function () {
  if (this.buttonContainer != null) {
    ...
    if (
      ...
      if (this.commentButton == null) {
        ...
        if (
          ...
          this.userElement.parentNode == this.buttonContainer
        ) {
          ...
          this.shareButton.parentNode == this.buttonContainer
        ) {
          this.buttonContainer.insertBefore(
            this.commentButton,
            this.shareButton
    ...
    if (
        ...
        if (this.shareButton != null) {
          ...
          if (
            ...
            this.shareButton.parentNode.style.display =
              this.shareButton.parentNode.clientWidth == 0 ? 'none' : ''
          }
        }
      ...
      // Fetch notifications
      if (urlParams['extAuth'] != '1' && Editor.currentTheme != 'atlas') {
        //Disable notification with external auth (e.g, Teams app)
        this.fetchAndShowNotification('online', this.mode)
      }
    } else {
      if (urlParams['notif'] != null) {
        //Notif for embed mode
      ...
      if (
        ...
        this.buttonContainer.style.display =
          this.buttonContainer.clientWidth == 0 ? 'none' : ''
      }
    }
...
App.prototype.fetchAndShowNotification = function (target, subtarget) {
  ...
  var processNotif = mxUtils.bind(this, function (notifs) {
    notifs = notifs.filter(function (notif) {
      return (
        !notif.targets ||
        notif.targets.indexOf(target) > -1 ||
        (subtarget != null && notif.targets.indexOf(subtarget) > -1)
      )
    })

    var lsReadFlag = target + 'NotifReadTS'
    var lastRead = isLocalStorage
      ? parseInt(localStorage.getItem(lsReadFlag))
      : true
...
App.prototype.showNotification = function (notifs, lsReadFlag) {
  ...
  function shouldAnimate(newNotif) {
    ...
    notifBell.className =
      'geNotification-bell' + (newNotif == 0 ? ' geNotification-bellOff' : '')
    document.querySelector('.geBell-rad').style.animation =
      newNotif == 0 ? 'none' : ''
  }
    ...
    mxEvent.addListener(
      ...
      mxUtils.bind(this, function () {
        if (this.notificationWin.style.display == 'none') {
          ...
          this.notificationWin.style.top =
            r.top + this.notificationBtn.clientHeight + 'px'
          this.notificationWin.style.left =
            r.right - this.notificationWin.clientWidth + 'px'
 ...
 */
App.prototype.pickFolder = function (
  mode,
  fn,
  enabled,
  direct,
  ...
  if (enabled && mode == App.MODE_GOOGLE && this.drive != null) {
    ...
    this.oneDrive.pickFolder(
      mxUtils.bind(this, function (files) {
        var folderId = null
        resume()
    ...
    this.gitHub.pickFolder(
      mxUtils.bind(this, function (folderPath) {
        resume()
        fn(folderPath)
      })
    )
  } else if (enabled && mode == App.MODE_GITLAB && this.gitLab != null) {
    this.gitLab.pickFolder(
      mxUtils.bind(this, function (folderPath) {
        resume()
        fn(folderPath)
      })
    ...
    this.trello.pickFolder(
      mxUtils.bind(this, function (cardId) {
        resume()
        fn(cardId)
      })
    )
 ...
 *
 */
App.prototype.exportFile = function (
  data,
  filename,
  mimeType,
  base64Encoded,
  ...
  if (mode == App.MODE_DROPBOX) {
    if (
      ...
      this.dropbox.insertFile(
        filename,
        base64Encoded ? this.base64ToBlob(data, mimeType) : data,
        mxUtils.bind(this, function () {
          this.spinner.stop()
        }),
        mxUtils.bind(this, function (resp) {
...
App.prototype.descriptorChanged = function () {
  ...
  if (file != null) {
    if (this.fname != null) {
      ...
      this.fname.setAttribute(
        'title',
        filename + ' - ' + mxResources.get('rename')
      )
    }
  ...
  if (
    this.format != null &&
    (file == null || this.fileEditable != file.isEditable()) &&
 ...
 * Adds the listener for automatically saving the diagram for local changes.
 */
App.prototype.showAuthDialog = function (
  peer,
  showRememberOption,
  fn,
  closeFn
  ...
  this.showDialog(
    new AuthDialog(
      ...
      peer,
      showRememberOption,
      mxUtils.bind(this, function (remember) {
        try {
          if (fn != null) {
            fn(
              remember,
 ...
 */
App.prototype.convertFile = function (
  url,
  filename,
  mimeType,
  extension,
  ...
  if (
    this.gitHub != null &&
    url.substring(0, this.gitHub.baseUrl.length) == this.gitHub.baseUrl
  ) {
    gitHubUrl = true
  }
  ...
  if (
    ...
    var handleData = mxUtils.bind(this, function (data) {
      try {
        if (/\.pdf$/i.test(filename)) {
          ...
          Graph.fileSupport &&
          new XMLHttpRequest().upload &&
          this.isRemoteFileFormat(data, url)
        ) {
          this.parseFileData(
            data,
            mxUtils.bind(this, function (xhr) {
    ...
    var binary =
      ...
      /\.pdf$/i.test(filename) ||
      (mimeType != null && mimeType.substring(0, 6) == 'image/')
...
App.prototype.updateHeader = function () {
  if (this.menubar != null) {
        ...
        if (file != null) {
          ...
          if (mode == App.MODE_GOOGLE) {
            this.appIcon.style.backgroundImage =
              'url(' + IMAGE_PATH + '/google-drive-logo-white.svg)'
            this.appIcon.style.backgroundSize = '70% 70%'
          } else if (mode == App.MODE_DROPBOX) {
            this.appIcon.style.backgroundImage =
              'url(' + IMAGE_PATH + '/dropbox-logo-white.svg)'
            this.appIcon.style.backgroundSize = '70% 70%'
          } else if (mode == App.MODE_ONEDRIVE) {
            this.appIcon.style.backgroundImage =
              'url(' + IMAGE_PATH + '/onedrive-logo-white.svg)'
            this.appIcon.style.backgroundSize = '70% 70%'
          } else if (mode == App.MODE_GITHUB) {
            this.appIcon.style.backgroundImage =
              'url(' + IMAGE_PATH + '/github-logo-white.svg)'
            this.appIcon.style.backgroundSize = '70% 70%'
          } else if (mode == App.MODE_GITLAB) {
            this.appIcon.style.backgroundImage =
              'url(' + IMAGE_PATH + '/gitlab-logo-white.svg)'
            this.appIcon.style.backgroundSize = '100% 100%'
          } else if (mode == App.MODE_TRELLO) {
            this.appIcon.style.backgroundImage =
              'url(' + IMAGE_PATH + '/trello-logo-white-orange.svg)'
            this.appIcon.style.backgroundSize = '70% 70%'
          }
        }
      })
     ...
     */
    var right =
      Editor.currentTheme != 'atlas' && urlParams['embed'] != '1' ? 10 : 10
    this.toggleFormatElement = document.createElement('a')
    this.toggleFormatElement.setAttribute(
      'title',
      mxResources.get('format') + ' (' + Editor.ctrlKey + '+Shift+P)'
    )
    this.toggleFormatElement.style.position = 'absolute'
    this.toggleFormatElement.style.display = 'inline-block'
    this.toggleFormatElement.style.top =
      Editor.currentTheme == 'atlas' ? '8px' : '6px'
    ...
    this.toggleFormatElement.className =
      Editor.currentTheme != 'atlas' ? 'geButton geAdaptiveAsset' : ''
    ...
    var toggleFormatPanel = mxUtils.bind(this, function () {
      if (this.formatWidth > 0) {
        this.toggleFormatElement.style.backgroundImage =
          "url('" + this.formatShowImage + "')"
      } else {
        this.toggleFormatElement.style.backgroundImage =
          "url('" + this.formatHideImage + "')"
      }
    })
    ...
    this.fullscreenElement.style.backgroundImage =
      "url('" + Editor.fullscreenImage + "')"
     ...
     */
    if (false && urlParams['embed'] != '1' && Editor.currentTheme != 'atlas') {
      ...
      this.toggleElement.style.backgroundImage =
        "url('" + this.chevronUpImage + "')"
      ...
      if (
        !mxClient.IS_FF &&
        screen.height <= 740 &&
        typeof this.toggleElement.click !== 'undefined'
      ) {
        window.setTimeout(
          mxUtils.bind(this, function () {
...
App.prototype.toggleCompactMode = function (visible) {
  ...
  if (this.toggleElement != null) {
    this.toggleElement.style.backgroundImage =
      "url('" + (visible ? this.chevronUpImage : this.chevronDownImage) + "')"
  }
...
App.prototype.updateUserElementStyle = function () {
  ...
  if (elt != null) {
    if (
      ...
      elt.style.height = ''
      elt.style.right =
        Editor.currentTheme == 'atlas' || this.darkModeElement != null
          ? '12px'
          : '26px'
...
App.prototype.toggleUserPanel = function () {
  ...
  if (this.userPanel.parentNode != null) {
      ...
      if (driveUsers.length > 0) {
        ...
        var createUserRow = mxUtils.bind(this, function (user) {
          ...
          if (user.isCurrent) {
            ...
            tr.setAttribute(
              'title',
              mxResources.get('login') + ' (' + 'User ID: ' + user.id + ')'
            )
    ...
    if (this.dropbox != null) {
      addUser(
        this.dropbox.getUser(),
        IMAGE_PATH + '/dropbox-logo.svg',
        mxUtils.bind(this, function () {
          var file = this.getCurrentFile()
