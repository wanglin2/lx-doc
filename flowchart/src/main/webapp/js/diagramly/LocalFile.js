...
LocalFile.prototype.isAutosave = function () {
  return (
    ...
    !this.invalidFileHandle &&
    DrawioFile.prototype.isAutosave.apply(this, arguments)
  )
}
...
LocalFile.prototype.getTitle = function () {
  return window.VueVm.fileName // this.title;
}
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
LocalFile.prototype.save = function (
  revision,
  success,
  error,
  unloading,
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
LocalFile.prototype.saveAs = function (
  title,
  success,
  error,
  unloading,
...
LocalFile.prototype.saveFile = function (
  ...
  var doSave = mxUtils.bind(this, function (data) {
    if (this.fileHandle != null) {
      ...
      if (!this.savingFile) {
        ...
        this.fileHandle.createWritable().then(
          mxUtils.bind(this, function (writable) {
            this.fileHandle.getFile().then(
              mxUtils.bind(this, function (newDesc) {
                ...
                EditorUi.debug(
                  ...
                  [newDesc],
                  'conflict',
                  this.desc.lastModified != newDesc.lastModified
                )
                ...
                if (
                  ...
                  writable
                    .write(
                      binary ? this.ui.base64ToBlob(data, 'image/png') : data
                    )
                    .then(
      ...
      if (this.ui.isOfflineApp() || this.ui.isLocalFileSave()) {
        this.ui.doSaveLocalFile(
          data,
          title,
          binary ? 'image/png' : 'text/xml',
          binary
        )
      } else {
  ...
  if (binary) {
    ...
    this.ui.getEmbeddedPng(
      mxUtils.bind(this, function (imageData) {
        doSave(imageData)
      }),
      error,
